...
import { BAIDU_FANYI } from "../conf.json";
...
const SECRET = BAIDU_FANYI.SECRET;
const FROM = ref("zh"),
    TO = ref("en");
...
const queryStr = () => {
    return `?q=${encodeQueryContent(origonalContent.value)}&from=${
        FROM.value
    }&to=${TO.value}&appid=${APP_ID}&salt=${salt}&sign=${createSign()}`;
};
...
<template>
    <main style="position: relative">
        ...
        <textarea
            class="quote-input"
            rows="4"
            v-model="origonalContent"
            placeholder="input your quote"
        ></textarea>
        <div style="margin-top: 1.2rem">
            <button type="button" @click="fetchTranslate">translate</button>
            <button type="button" @click="download" style="margin-left: 2rem">
                download pic
            </button>
        </div>
        <div
        ...
        >
            <p
                class="im-fell-english-regular inner-content"
                :data-text="origonalContent"
